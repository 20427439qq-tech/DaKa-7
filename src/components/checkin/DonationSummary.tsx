...
export const DonationSummary: React.FC<DonationSummaryProps> = ({ amount, completedCount }) => {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className={`text-[10px] mt-0.5 font-bold ${isPerfect ? 'text-emerald-100' : 'text-red-400'}`}>
              {isBefore10PM ? '22点前显示为0，结算后显示实际金额' : '超过22点未打卡需乐捐9000元'}
            </p>
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="flex items-end gap-1">
            ...
            <span className={`text-sm mb-1 ${isPerfect ? 'text-emerald-100' : 'text-gray-400'}`}>/ 7</span>
          </div>
        </div>
      </div>
